...
triplets = zeros(Int, 83, 83, 83)
for msg in messages
    for i in 1:length(msg) -2
        right = msg[i+1]
        right2 = msg[i+2]
        triplets[msg[i], right, right2] += 1
    end
end

quads = zeros(Int, 83, 83, 83, 83)
for msg in messages
    for i in 1:length(msg) -3
        right = msg[i+1]
        right2 = msg[i+2]
        right3 = msg[i+3]
        quads[msg[i], right, right2, right3] += 1
    end
end


open("pairs.txt", "w") do f
    println(f, "i\tj\tvalor") 
    for i in 1:83
        for j in 1:83
            println(f, i, "\t", j, "\t", pairs[i, j])
        end
    end
end

open("triplets.txt", "w") do f
    println(f, "i\tj\tk\tvalor")
    for i in 1:83
        for j in 1:83
            for k in 1:83
                println(f, i, "\t", j, "\t", k, "\t", triplets[i, j, k])
            end
        end
    end
end

open("quads.txt", "w") do f
    println(f, "i\tj\tk\tl\tvalor")
    for i in 1:83
        for j in 1:83
            for k in 1:83
                for l in 1:83
